...
class Alternate:
    ...
    def run(self, vm: Vandermonde, rating_mat_tr, rating_mat_te, n_iter, logger: Logger=None):
        for it in range(n_iter):
            ...
            # Test rmse
            rmse_tr = self.calc_prediction_rmse(vm, a_mat, rating_mat_tr)
            rmse_te = self.calc_prediction_rmse(vm, a_mat, rating_mat_te)

            if isinstance(logger, Logger):
                logger.log(rmse_tr, rmse_te)

    @staticmethod
    def calc_prediction_rmse(vm: Vandermonde, a_mat, rating_mat_te):
